...
class StartMenu extends Phaser.Scene {
    ...
    create() {
        ...
        // High Scores Button
        const highScoresButton = this.add.text(config.width / 2, config.height / 2 + 80, 'HIGH SCORES', {
            fontSize: '32px',
            color: '#fff',
            fontFamily: 'monospace',
            backgroundColor: '#444',
...
class HighScores extends Phaser.Scene {
    ...
    create() {
        ...
        highScores.forEach((score, index) => {
            highScoreText += `${index + 1}. ${score.name}: ${score.playerScore}-${score.aiScore} (${score.time}s) - ${score.date}\n`;
        });

        if (highScores.length === 0) {
            highScoreText = 'No scores yet!';
        }

        this.add.text(config.width / 2, config.height / 2, highScoreText, {
            fontSize: '24px',
            color: '#fff',
            fontFamily: 'monospace',
            align: 'center'
        ...
        // Back Button
        const backButton = this.add.text(config.width / 2, config.height - 50, 'BACK', {
            fontSize: '32px',
            color: '#fff',
            fontFamily: 'monospace',
            backgroundColor: '#444',
