...
export default class OptionsScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Title
        const title = this.add.text(config.width / 2, 50, 'OPTIONS', {
            ..._.styles.text,
            fontSize: '48px',
        }).setOrigin(0.5);
    ...
    createVolumeSection(label, y, isMusic) {
        // Label
        const labelText = this.add.text(config.width / 2 - 200, y, `${label} VOLUME:`, {
            ..._.styles.text,
            fontSize: '24px',
        }).setOrigin(0, 0.5);
        ...
        // Slider background
        const slider = this.add.rectangle(config.width / 2, y + 30, 400, 20, 0x444444);
        ...
        const toggleButton = _.createButton(
            ...
            () => {
                if (isMusic) {
                    ...
                    toggleButton.setText(`${label}: ${this.audio.musicMuted ? 'OFF' : 'ON'}`);
                } else {
                    this.audio.toggleSFXMute();
                    toggleButton.setText(`${label}: ${this.audio.sfxMuted ? 'OFF' : 'ON'}`);
                }
